    ...
    void start() {
        ...
        server_thread_ = std::thread([this]{
         ...
         svr.Get("/", [this](const httplib::Request& req, httplib::Response& res){
                std::string index_path = static_dir_ + "/index.html";
                std::cout << "Attempting to serve: " << index_path << std::endl;
                
                std::ifstream ifs(index_path);
                if (!ifs.is_open()) {
                    std::cout << "ERROR: Could not open index.html at: " << index_path << std::endl;
                    res.status = 404;
                    res.set_content("index.html not found at: " + index_path, "text/plain");
                    return;
                }
                ...
                std::cout << "Served index.html successfully" << std::endl;
            });
